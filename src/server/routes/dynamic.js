...
router.get('/tuning/:brand/:model', (req, res, next) => {
  ...
  res.render('templates/vehicle-page', {
    title: `${vehicle.brand} ${vehicle.model} Kennfeldoptimierung — +${psGain} PS | Vmax Sued`,
    description: `${vehicle.brand} ${vehicle.model} Kennfeldoptimierung in Westendorf bei Augsburg. Einzelabstimmung auf dem Prüfstand: von ${vehicle.stock.ps} PS auf ${vehicle.stage1.ps} PS (Stage 1). TÜV-Eintragung möglich.`,
    vehicle,
    otherVehicles,
    schema: {
      name: `${vehicle.brand} ${vehicle.model} Kennfeldoptimierung`,
    ...
    breadcrumbs: [
      { name: 'Kennfeldoptimierung', path: '/chiptuning' },
      { name: `${vehicle.brand} ${vehicle.model}`, path: `/tuning/${vehicle.brandSlug}/${vehicle.slug}` },
    ],
    faq: [
      { q: `Wie viel PS bringt ein Kennfeldoptimierung beim ${vehicle.model}?`, a: `Bei einer Stage-1-Einzelabstimmung erzielen wir typischerweise +${psGain} PS und +${nmGain} Nm Drehmoment.` },
      ...
      { q: `Was kostet das Kennfeldoptimierung für den ${vehicle.model}?`, a: `Die Kosten für eine Stage-1-Abstimmung beginnen bei 499 €. Kontaktieren Sie uns für ein individuelles Angebot.` },
    ],
  });
});
...
router.get('/tuning/:city', (req, res, next) => {
  ...
  res.render('templates/city-page', {
    title: `Kennfeldoptimierung ${city.name} — Vmax Sued | ${city.distance} entfernt`,
    description: `Kennfeldoptimierung und Leistungssteigerung für ${city.name} und Umgebung. Vmax Sued in Westendorf — nur ${city.distance} (${city.driveTime}) von ${city.name} entfernt.`,
    city,
    vehicles: vehicles.slice(0, 6).map(v => ({ ...v, brandSlug: v.brand.toLowerCase().replace(/\s+/g, '-').replace(/š/g, 's') })),
    schema: {
      name: `Kennfeldoptimierung ${city.name}`,
      description: `Professionelles Kennfeldoptimierung für Kunden aus ${city.name}. Einzelabstimmung auf dem Prüfstand bei Vmax Sued in Westendorf — ${city.distance} entfernt.`,
    },
    breadcrumbs: [
      { name: 'Kennfeldoptimierung', path: '/chiptuning' },
